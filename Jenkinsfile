    ...
    stages {
        ...
        stage('ci') {
            ...
            steps {
                ...
                sh "sed -i -- 's/REPLACE_PORT/80/g' *template.yml"
                sh "sed -i -- 's/REPLACE_PROTOCOL/http/g' *template.yml"
                sh "sed -i -- 's/REPLACE_USER/${env.CI_SFTP_USR}/g' *template.yml"
                sh "sed -i -- 's/REPLACE_PASSWORD/${env.CI_SFTP_PSW}/g' *template.yml"
                sh "sed -i -- 's/REPLACE_URL/${env.DEV_SFTP_URL}/g' *template.yml"
                sh "sed -i -- 's/ENDPOINT_ENABLED/'false'/g' *template.yml"
                sh "sed -i -- 's/true/'true'/g' *template.yml"
        ...
        stage('test') {
            ...
            steps {
                ...
                sh 'sh artifactory-get.sh -r http://artifactory-sdc.onsdigital.uk/artifactory/libs-release-local/ -g uk.gov.ons.ctp.product -a samplesvc > target/samplesvc.jar'
                sh 'chmod 777 target/samplesvc.jar'
                sh "sed -i -- 's/SPACE/dev/g' *template.yml"
                sh "sed -i -- 's/INSTANCES/1/g' *template.yml"
                sh "sed -i -- 's/DATABASE/rm-pg-db/g' *template.yml"
                sh "sed -i -- 's/DOMAIN/${env.CF_DOMAIN}/g' *template.yml"
