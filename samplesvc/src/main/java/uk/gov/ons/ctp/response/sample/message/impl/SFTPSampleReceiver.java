...
/**
 * Created by wardlk on 13/04/2017.
 */
@Slf4j
@MessageEndpoint
public class SFTPSampleReceiver {
    ...
    public Message<String> processInvalidSample(GenericMessage errorMessage) throws CTPException, IOException {
        ...
        String directory = ((MessagingException) errorMessage.getPayload()).getFailedMessage().getHeaders().get("sample_type").toString() + "-sftp";
        ...
        final Message<String> message = MessageBuilder.withPayload(error).setHeader( "error_file_name",errorFile)
                .setHeader("file_name",fileName).setHeader("short_file_name", shortFileName).setHeader("directory",directory).build();
